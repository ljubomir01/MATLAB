...
n = 0:N_fft/2-1;
w = n*(fo/2)/(N_fft/2-1);
subplot (3, 1, 1); plot (f, Ba); title ('Amplitudska karakteristika VF filtra projektovanog koriscenjem Kajzerovog prozora - linearna osa');
subplot (3, 1, 2); plot (f, 20*log10(Ba)); title ('Amplitudska karakteristika VF filtra projektovanog koriscenjem Kajzerovog prozora - logaritamska osa');
subplot (3, 1, 3); plot (f, F); title ('Fazna karakteristika VF filtra projektovanog koriscenjem Kajzerovog prozora');
xlim([0 pi])
